 ...
 * `<div contenteditable>`, or select() on a custom dropdown that isn't a native
...
final class UndrivableElementException extends RuntimeException
{
    ...
    {
        $needs = match ($kind) {
            'value' => 'has no value to set — fill()/type()/clear() need an <input> or <textarea>;'
                .' drive a custom widget with click()/type() or evaluate()',
